...
public class PayChannelServiceImpl extends BaseService implements IPayChannelService {
    ...
    @Override
    public List<PayChannel> getEnablePayChannelListByMchId(String mchId) {
        List<PayChannel> payChannelList = super.baseSelectEnablePayChannelList(mchId);
        return payChannelList;
    }

    @Override
    public PayChannel selectPayChannelEntity(String jsonParam) {
        BaseParam baseParam = JsonUtil.getObjectFromJson(jsonParam, BaseParam.class);
        Map<String, Object> bizParamMap = baseParam.getBizParamMap();
        if (ObjectValidUtil.isInvalid(bizParamMap)) {
            _log.warn("查询支付渠道信息失败, {}. jsonParam={}", RetEnum.RET_PARAM_NOT_FOUND.getMessage(), jsonParam);
    ...
    public PayChannel getEntityByMchIdAndChannelId(String mchId, String channelId) {
        Map<String,Object> paramMap = new HashMap<>();
        paramMap.put("mchId", mchId);
        paramMap.put("channelId", channelId);
        String jsonParam = RpcUtil.createBaseParam(paramMap);
        PayChannel payChannel = selectPayChannelEntity(jsonParam);
        return payChannel;
    }
